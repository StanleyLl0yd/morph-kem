#!/usr/bin/env python3
from __future__ import annotations

import argparse
from pathlib import Path
import shutil
import subprocess
import tempfile
import time

from morph_kem.gluing_stacked_sphere import (
    G14_PARAMETER_SETS,
    decode_stacked_sphere_sat_model,
    encode_stacked_sphere_sat,
    generate_stacked_sphere_instance,
)


MASTER_SEED = bytes.fromhex("76140450aabbccddeeff001122334455")


def _normalize(groups: tuple[tuple[int, ...], ...]) -> tuple[tuple[int, ...], ...]:
    return tuple(sorted(tuple(sorted(group)) for group in groups))


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Cross-check MORPH-KEM G14 stacked-sphere P3 relation with MiniSat."
    )
    parser.add_argument("--params", choices=sorted(G14_PARAMETER_SETS), default="g14-72")
    parser.add_argument("--solver", default="minisat")
    args = parser.parse_args()

    solver = shutil.which(args.solver)
    if solver is None:
        parser.error(f"SAT solver not found: {args.solver}")

    public, reference = generate_stacked_sphere_instance(
        G14_PARAMETER_SETS[args.params], MASTER_SEED
    )
    encoding = encode_stacked_sphere_sat(public)
    dimacs = encoding.to_dimacs()

    print(f"parameters: {args.params}")
    print(f"reference cover available: {reference is not None}")
    print(f"solver: {solver}")
    print(f"SAT variables: {encoding.variable_count}")
    print(f"SAT clauses: {len(encoding.clauses)}")
    print(f"DIMACS bytes: {len(dimacs.encode('ascii'))}")

    with tempfile.TemporaryDirectory(prefix="morph-g14-sat-") as directory:
        cnf_path = Path(directory) / "g14.cnf"
        model_path = Path(directory) / "g14.model"
        cnf_path.write_text(dimacs, encoding="ascii")

        started = time.perf_counter()
        completed = subprocess.run(
            [solver, str(cnf_path), str(model_path)],
            capture_output=True,
            text=True,
            check=False,
        )
        elapsed = time.perf_counter() - started

        print(f"solver return code: {completed.returncode}")
        print(f"solver elapsed seconds: {elapsed:.6f}")
        for line in completed.stdout.splitlines():
            if any(
                label in line
                for label in ("restarts", "conflicts", "decisions", "propagations", "CPU time")
            ):
                print(f"solver {line.strip()}")

        if completed.returncode == 20:
            print("solver result: UNSAT")
            if reference is not None:
                print("ERROR: generator already found a valid reference cover")
                return 2
            return 0
        if completed.returncode != 10:
            print("solver result: UNKNOWN/ERROR")
            if completed.stderr:
                print(completed.stderr.strip())
            return 3
        if not model_path.exists():
            print("solver result: SAT without model file")
            return 4

        tokens = model_path.read_text(encoding="ascii").split()
        if not tokens or tokens[0] != "SAT":
            print("solver result: malformed model")
            return 5
        literals = tuple(int(token) for token in tokens[1:] if token != "0")
        model = decode_stacked_sphere_sat_model(public, encoding, literals)
        print("solver result: SAT")
        print(f"selected candidate pieces: {len(model.groups)}")
        print(f"decoded accepted witness: {model.validation.valid}")
        if reference is None:
            print("decoded witness matches reference after public success: n/a")
        else:
            print(
                "decoded witness matches reference after public success: "
                f"{_normalize(model.groups) == _normalize(reference.groups)}"
            )
        return 0 if model.validation.valid else 6


if __name__ == "__main__":
    raise SystemExit(main())
