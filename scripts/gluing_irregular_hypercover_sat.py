...
from morph_kem.gluing_irregular_hypercover import (
    G12_PARAMETER_SETS,
    decode_irregular_hypercover_sat_model,
    encode_irregular_hypercover_sat,
    generate_irregular_hypercover_instance,
    irregular_hypercover_reference_matches,
)
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Cross-check MORPH-KEM G12 irregular P3 hypercover with MiniSat."
    )
    ...
    public, reference = generate_irregular_hypercover_instance(
        G12_PARAMETER_SETS[args.params], MASTER_SEED
    )
    ...
    with tempfile.TemporaryDirectory(prefix="morph-g12-sat-") as directory:
        cnf_path = Path(directory) / "g12.cnf"
        model_path = Path(directory) / "g12.model"
        ...
        if completed.returncode == 20:
            ...
            print("ERROR: generated G12 instance has a known valid reference cover")
            return 2
        ...
        model = decode_irregular_hypercover_sat_model(public, encoding, literals)
        ...
        print(
            ...
            f"{irregular_hypercover_reference_matches(reference, model.groups)}"
        )
