...
def main() -> int:
    parser = argparse.ArgumentParser(description="Sweep MORPH-KEM G11 public P3 exact-cover recovery.")
    ...
    print(
        "set,seed,V,E,F,dual_edges,bridges,articulations,bipart_left,bipart_right,"
        "candidates,membership_hist,overlap_hist,candidate_incidence,cover_solutions,"
        "cover_cap_hit,cover_nodes,cover_decisions,cover_backtracks,accepted,nonreference"
    )
    failures = 0
    for name in sorted(G11_PARAMETER_SETS):
        params = G11_PARAMETER_SETS[name]
        for seed_index in range(args.seeds):
            ...
            left, right = recovery.bipartition_sizes
            print(
                f"{name},{seed_index},{incidence.vertices},{incidence.edges},{incidence.triangles},"
                f"{recovery.dual_edges},{recovery.bridge_count},{len(recovery.articulation_points)},"
                f"{left},{right},{recovery.candidate_count},"
            ...
            ok = (
                ...
                and recovery.candidate_count == 3 * triangle_count
                and recovery.candidate_membership_histogram == ((9, triangle_count),)
                and recovery.candidate_overlap_degree_histogram == ((18, 3 * triangle_count),)
                and recovery.candidate_triangle_incidence == 9 * triangle_count
