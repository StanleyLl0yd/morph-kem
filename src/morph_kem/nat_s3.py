...
_TRANSPOSITIONS: tuple[Permutation, ...] = (
    (1, 0, 2),
    (2, 1, 0),
    (0, 2, 1),
)
...
def _carrier(params: NAT2Parameters, seed: bytes) -> tuple[SimplicialComplex, int]:
    ...
    mixed.validate()
    growth_seed = _digest(b"MORPH-KEM NAT2 growth v1", seed, params.name)
    flip_seed = _digest(b"MORPH-KEM NAT2 flips v1", seed, params.name)
    relabel_seed = _digest(b"MORPH-KEM NAT2 relabel v1", seed, params.name)
...
def _normalized_hidden_labels(
    ...
    labels = tuple(
        _S3[int.from_bytes(_digest(b"MORPH-KEM NAT2 hidden S3 v1", seed, name, vertex)[:8], "big") % len(_S3)]
...
def _seeded_noise_edges(edge_count: int, weight: int, seed: bytes, name: str) -> tuple[int, ...]:
    order = sorted(
        range(edge_count),
        key=lambda index: (_digest(b"MORPH-KEM NAT2 noise edge v1", seed, name, index), index),
    )
    return tuple(sorted(order[:weight]))


def generate_nat2_instance(
    params: NAT2Parameters,
    master_seed: bytes,
    noise_weight: int,
    ...
    hidden = _normalized_hidden_labels(target, master_seed, params.name + f"-w{noise_weight}")
    clean = list(_clean_edge_labels(target, hidden))
    noise_edges = _seeded_noise_edges(len(clean), noise_weight, master_seed, params.name + f"-w{noise_weight}")
    noise_labels: list[Permutation] = []
    for edge_index in noise_edges:
        digest = _digest(
            b"MORPH-KEM NAT2 transposition v1",
            master_seed,
            params.name + f"-w{noise_weight}",
            edge_index,
        )
        transposition = _TRANSPOSITIONS[int.from_bytes(digest[:8], "big") % 3]
        clean[edge_index] = compose(clean[edge_index], transposition)
        noise_labels.append(transposition)

    return (
        NAT2Public(params.name, target, tuple(clean), noise_weight),
...
def _spanning_tree(target: SimplicialComplex) -> tuple[tuple[int, int, int], ...]:
    edges = _edges(target)
    adjacency: dict[int, list[tuple[int, int]]] = {vertex: [] for vertex in target.vertices}
...
def _derive_child_label(
    ...
    transposition: Permutation | None,
) -> Permutation:
    clean = observed if transposition is None else compose(observed, transposition)
    left, right = edge
    if parent_vertex == left and child_vertex == right:
        return compose(parent_label, clean)
    if parent_vertex == right and child_vertex == left:
        return compose(parent_label, inverse(clean))
    raise NAT2Error("NAT2 spanning-tree edge orientation mismatch")
...
def _exact_state_search(
    ...
    *,
    solution_cap: int = 16,
) -> tuple[tuple[tuple[Permutation, ...], tuple[int, ...]], ...], int, int, bool]:
    ...
    tree_edge_indices = {edge_index for _, _, edge_index in tree}
    non_tree_edges = tuple(
        edge_index for edge_index in range(len(edges)) if edge_index not in tree_edge_indices
    )

    labels: list[Permutation | None] = [None] * len(vertices)
    labels[0] = _IDENTITY
    solutions: list[tuple[tuple[Permutation, ...], tuple[int, ...]]] = []
    nodes = 0
    backtracks = 0
    cap_hit = False

    def finish(tree_noise_edges: tuple[int, ...]) -> None:
        nonlocal backtracks, cap_hit
        if any(label is None for label in labels):
            raise NAT2Error("NAT2 exact search reached an incomplete assignment")
        concrete = tuple(labels)  # type: ignore[arg-type]
        noisy = list(tree_noise_edges)
        ...
        if not accepted:
            raise NAT2Error("NAT2 exact search produced a verifier-rejected state")
        solutions.append((concrete, verifier_noise))
        if len(solutions) >= solution_cap:
            cap_hit = True

    def search(tree_index: int, noise_edges: tuple[int, ...]) -> None:
        ...
        if parent_label is None:
            raise NAT2Error("NAT2 spanning-tree order is not parent-first")

        options: tuple[Permutation | None, ...] = (None,) + _TRANSPOSITIONS
        progressed = False
        for transposition in options:
            extra = 0 if transposition is None else 1
            if len(noise_edges) + extra > public.public_noise_weight:
                continue
            child_label = _derive_child_label(
                parent,
                child,
                edges[edge_index],
                parent_label,
            ...
            search(
                tree_index + 1,
                noise_edges if transposition is None else noise_edges + (edge_index,),
            )
            labels[child] = None
            if cap_hit:
                return
...
def recover_nat2(
    ...
    *,
    ...
    sign_clean_accepted = True

    sign_match_noise = None
    sign_match_hidden = None
    if reference is not None:
        planted_noise_mask = sum(1 << edge for edge in reference.planted_noise_edges)
        sign_match_noise = correction == planted_noise_mask
    ...
    return NAT2Recovery(
        ...
        triangles=len(tuple(simplex for simplex in public.target.simplices if len(simplex) == 3)),
        ...
        sign_clean_parity_accepted=sign_clean_accepted,
