...
def reduce_involutive_word(word: Word) -> Word:
    """Cancel adjacent equal involutions without using any hidden information."""
    stack: list[int] = []
...
def bidirectional_bfs(
    ...
    while right_parents[current] is not None:
        ...
        current = parent
    # The two individually shortest halves may meet with the same involution label
    # at the join.  Publicly cancel such Vi*Vi pairs before calling the reduced-word
    # verifier; this does not use the planted action.
    word = reduce_involutive_word(left_word + tuple(right_word))
    return NavigationRecovery(
        verify_action(prime, source, target, word_bound, word),
        word,
        len(word),
